...
function deriveInstruction(state: FaceScanState): { primary: string; sub: string | null } {
...
export function PoseInstruction({ state }: Props) {
  ...
  const primaryColor =
    isCaptured                       ? C.captured :
    state.poseStatus === 'stabilizing' ? C.success :
    C.textPrimary;

  return (
    <Animated.View style={[styles.container, { opacity: fadeAnim }]}>
      <Text style={[styles.primary, { color: primaryColor }]}>{display.primary}</Text>
      {display.sub ? (
